...
% load('D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_grp_task_effect_Tlocked\level2_RM_ANOVA_HEP_grp_task_effect_50-400ms\mask_group_effect')
%  
% [row, column] = find(mask == 1);
% older_all_mean = squeeze(mean(HEP{2}(:, 1:59, :), 1));
% avg_ampl_cluster1 = mean(mean(older_all_mean(row, column + 125), 1))
