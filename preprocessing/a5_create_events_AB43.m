% create R and T events for subject AB43 - ECG with unsual shape not
% recognized by toolbox
% latency in seconds and event type Rpeak or Tpeak
% Maria Ribeiro
clear; close all
% Data from directory
eeg_data_dir = 'L:\ProjectAgeingAuditoryTask\BIDS\sub-AB43\eeg';
...
for f = 1:length(dirData)
    if contains(dirData(f).name, 'eeg.set')
        ...
        [pks, locs] = findpeaks(data_ecg, 'MinPeakProminence',50);
        
        zscore_pks = zscore(pks);
        Rpks = pks(zscore_pks < 0);
        Rlocs = locs(zscore_pks < 0);
        Tpks = pks(zscore_pks > 0);
        Tlocs = locs(zscore_pks > 0);
        ...
        filepath = 'L:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\neweeg_events\sub-AB43';
