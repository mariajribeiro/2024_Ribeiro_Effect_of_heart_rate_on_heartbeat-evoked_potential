...
clear; close all
save_dir = 'E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\eeg_derivatives_05Hzfilt_cardiacICs';
save_dir_eeg_whole = [save_dir, filesep, 'no_removed_segments'];
...
%     fileList2 = {nextdirData2(~nextdirIndex2).name}';
  ...
  % load file with artefact components
   str = [dirData_art_ics(1).folder,filesep, [sbj_included{s}(5:end), '_artifact_ICs.mat']];
   load(str);
   % load file with cardiac components calculated in E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\eeg_preproc_cardiac_ics.m
   load([pwd, '\cardiac_ICs\' sbj_included{s}(5:end), '_cardiac_ICs.mat']);
   ...
   for k = 1:length(fileList_eeg)
       if contains(fileList_eeg(k), 'eeg.set')
           ...
           EEG = pop_subcomp( EEG, [artifact_ICs, cardiac_ics{5}] , 0);
...
%             EEG = pop_importevent( EEG, 'append','no','event',fullfilename_events{1},'fields',{'latency','type'},'skipline',1,'timeunit',1);
           ...
           basefilename = sprintf('eeg_full_Rpeak_Tpeak_%s', fileList_eeg{k});
           EEG = pop_saveset( EEG, 'filename',basefilename,'filepath', save_dir_eeg_whole);
