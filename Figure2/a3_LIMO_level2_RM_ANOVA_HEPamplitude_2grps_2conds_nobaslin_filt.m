...
level2_RM_ANOVA_dir = [pwd, '\level2_RM_ANOVA_HEP_grp_task_effect_50-400ms'];
...
for f = 1:length(folder_list)
    if contains(folder_list(f).name, 'AB')
        ...
        Y(:, :, s, :) = Betas(:, 126:300, 1:2); % channels, time, subjects, measures - from -200 up to 350 locked with T peak
