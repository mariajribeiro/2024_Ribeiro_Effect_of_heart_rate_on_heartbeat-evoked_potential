% for each participant run a correlation between HEP and IBI on a
...
%     end
%     
    HEP_T_locked_data = []; IBI = cell(2, 1); number_trials = [];
...
%                 EEG = pop_select( EEG, 'nochannel',{'EKG'});
        ...
        if t_peaks ~= 0 % in case t peaks were detected
            ...
            HEP_T_locked_data = cat(3, HEP_T_locked_data, EEG.data(1:59, 126:end, 1:number_trials(1)));
    ...
    if sum(number_trials) > 0    
        ...
        % use same number of trials for all tasks - should be smaller for
        % passive task - use first trials for simple RT and gng up to
        % same number as passive
        categorical_variable=[ones(number_trials(1), 1); ones(number_trials(1), 1)*2; ones(number_trials(1), 1)*3];

        % create continuous variable with IBI data for each task condition in
        % different columns
        % z-score IBI data
        for t = 1:length(tasks)
            zscore_ibi{t} = zscore(IBI{t}(1:number_trials(1)));
        end

        continuous_variable = zeros(length(categorical_variable), 2);
        continuous_variable(1:number_trials(1), 1) = zscore_ibi{1};
        continuous_variable(number_trials(1)+1:number_trials(1)+number_trials(1), 2) = zscore_ibi{2};
        continuous_variable(2*number_trials(1)+1:3*number_trials(1), 3) = zscore_ibi{3};
