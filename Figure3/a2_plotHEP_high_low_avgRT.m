...
groups = {'young', 'older'};
dir_RT = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_nobaseline_05HzFilter\level2_regression_RT_avgtasks';
for grp = 1:2
    ...
    channels_num = 14%[7 16 32 34 41 50 51];%[7, 25, 43, 57];
    for chn = channels_num
        ...
        if exist([mask_dir, '\mask_covariate.mat'])
            ...
            plot(x_axis(find(mask(chn, :) > 0) + 125), ones(length(find(mask(chn, :) >0)), 1) * -1.25, 'k', 'LineWidth', 4.5)
            hold on
        end
...
%% gng task
groups = {'young', 'older'};
dir_RT = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_nobaseline_05HzFilter\level2_regression_HR';
for grp = 1:2
