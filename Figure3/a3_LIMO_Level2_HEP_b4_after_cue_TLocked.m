...
files_dir = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\results_random_events';
...
% load HEPs
heps_dir ='D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study';
load([heps_dir, filesep, 'HEP_T_locked_aftercue']); 
load([heps_dir, filesep, 'HEP_T_locked_b4cue']); 
load([heps_dir, filesep, 'HEP_T_locked_all']);
...
y_tmp{1} = NaN(59, length(126:300), length(HEP_T_b4cue_detrended{1, 1}), 4);
y_tmp{2} = NaN(59, length(126:300), length(HEP_T_b4cue_detrended{2, 1}), 4);


for grp = 1:2
    for s = 1:size(HEP_T_b4cue_detrended{grp, 2}, 1)
        % simple RT before cue
        y_tmp{grp}(:, :, s, 1) = HEP_T_b4cue_detrended{grp, 2}(s, 2, :, 176:end); % analyse only from 51ms onwards
        % simple RT after cue
        y_tmp{grp}(:, :, s, 2) = HEP_T_aftercue_detrended{grp, 2}(s, 2, :, 176:end);
        % gng before cue
        y_tmp{grp}(:, :, s, 3) = HEP_T_b4cue_detrended{grp, 2}(s, 3, :, 176:end);
        % gng after cue
        y_tmp{grp}(:, :, s, 4) = HEP_T_aftercue_detrended{grp, 2}(s, 3, :, 176:end);
...
%%
limo_display_results(1,'Rep_ANOVA_Interaction_Factors_12.mat',pwd,0.05, 2,...
    fullfile(pwd,'LIMO.mat'), 0);
%%
...
for t = 2:3
    ...
    y1 = []; y2 = [];
    for s = 1:size(HEP_T_b4cue_detrended{grp, 2}, 1)
        y1(:, :, s) = mean(HEP_T_b4cue_detrended{1, 2}(s, t, :, 126:300));
        y2(:, :, s) = mean(HEP_T_aftercue_detrended{1, 2}(s, t, :, 126:300));
    end
...
%     limo_random_robust(3,y1,y2,parameter number,LIMO)
    ...
    mask = [];
    % limo_display_results(Type,FileName,PathName,p,MCC,LIMO,flag,options)
    limo_display_results(1,'paired_samples_ttest_parameter_1',pwd,0.05, 1,...
        fullfile(pwd,'LIMO.mat'), 0);
    % saveas(gcf, 'One_sample_timecourse.fig'); close(gcf)
    if ~isempty(mask)
        save mask mask
...
chan_dir = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study';
