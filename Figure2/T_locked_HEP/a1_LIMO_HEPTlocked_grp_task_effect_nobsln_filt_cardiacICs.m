...
%         EEG = pop_rmbase( EEG, [-150 -50] ,[]);
        ...
        if t_peaks ~= 0 % in case t peaks were detected
            ...
            [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
            % HEP T-locked data
            HEP_T_locked_data = cat(3, HEP_T_locked_data, EEG.data(1:59, 126:275, :));
...
mean_yng_RT_simpleRT = mean(RT_interval{1, 1}(:, 2))
mean_yng_RT_gng = mean(RT_interval{1, 2}(:, 2))

mean_old_RT_simpleRT = mean(RT_interval{2, 1}(:, 2))
mean_old_RT_gng = mean(RT_interval{2, 2}(:, 2))



mean_yng_RT = mean([mean(RT_interval{1, 1}(:, 2)), mean(RT_interval{1, 2}(:, 2))])
mean_old_RT = mean([mean(RT_interval{2, 1}(:, 2)), mean(RT_interval{2, 2}(:, 2))])


yng_RT = mean([RT_interval{1, 1}(:, 2), RT_interval{1, 2}(:, 2)], 2);

old_RT = mean([RT_interval{2, 1}(:, 2), RT_interval{2, 2}(:, 2)], 2);


[h,p,ci,stats] = ttest2(yng_RT, old_RT)


mean_yng = mean(yng_RT)
std_yng = std(yng_RT)

mean_old = mean(old_RT)
std_old = std(old_RT)
