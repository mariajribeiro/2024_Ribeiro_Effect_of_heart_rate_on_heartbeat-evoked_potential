...
% effect of task and group on HEP amplitude - beta
...
for f = 1:length(folder_list)
    if contains(folder_list(f).name, 'AB')
        ...
        Y(:, :, s, :) = Betas(:, :, 1:2); % channels, time, subjects, measures - from 51 up to 350 locked with T peak
...
if ~isempty(mask)
    ...
    load('E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\expected_chanlocs_both.mat');
    chan_sig = find(sum(mask, 2) > 0);
    chansignames = {};
    for ch = 1:length(chan_sig)
        chansignames{ch} = expected_chanlocs(chan_sig(ch)).labels;
    end
...
% task effect
load mask_main_effect
[name, cluster] = limo_get_effect_size('Rep_ANOVA_Main_effect_1.mat', mask)
