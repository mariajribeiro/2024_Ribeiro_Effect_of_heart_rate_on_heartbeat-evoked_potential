...
%                 EEG = pop_select( EEG, 'nochannel',{'EKG'});
        ...
        EEG = pop_epoch( EEG, {  'Rpeak'  }, [-0.15         3], 'newname', 'Rpeak epochs all', 'epochinfo', 'yes');
%         EEG = pop_rmbase( EEG, [-150 -50] ,[]);
        ...
        if t_peaks ~= 0 % in case t peaks were detected
            EEG = pop_epoch( EEG, {  'Tpeak'  }, [-0.2         1], 'newname', 'Tpeak epochs b4cue', 'epochinfo', 'yes');
            [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
            % HEP T-locked data
            ...
            for chn = 1:59
                avg_HEP = squeeze(mean(EEG.data(chn, :, :), 2));
                [b,~,~, ~,stats] = regress(avg_HEP,[ones(length(IBI), 1), IBI]);
                coef_HEP_IBI_tmp(chn, 1) = b(2);
            end 
        ...
        if group == 1
            ...
            coef_HEP_IBI{group, t}(yng, t, :) = coef_HEP_IBI_tmp;
        else
            ...
            coef_HEP_IBI{group, t}(old, t, :) = coef_HEP_IBI_tmp;
        end   
    end
end

save HEP_T_locked_IBI_tertiles_cardiacICs HEP_T_locked_IBI_tertiles
save coef_HEP_IBI_filt_cardiacICs coef_HEP_IBI

%% analyses regression coefficients

[h,p,ci,stats] = ttest(coef_HEP_IBI{1, 1}(:, 1, 34))
[h,p,ci,stats] = ttest(coef_HEP_IBI{1, 2}(:, 2, 34))
[h,p,ci,stats] = ttest(coef_HEP_IBI{1, 3}(:, 3, 34))

[h,p,ci,stats] = ttest(coef_HEP_IBI{2, 1}(:, 1, 34))
[h,p,ci,stats] = ttest(coef_HEP_IBI{2, 2}(:, 2, 34))
[h,p,ci,stats] = ttest(coef_HEP_IBI{2, 3}(:, 3, 34))


[h,p,ci,stats] = ttest2(coef_HEP_IBI{1, 2}(:, 2, 34), coef_HEP_IBI{2, 2}(:, 2, 34))

[h,p,ci,stats] = ttest(coef_HEP_IBI{1, 2}(:, 2, 34), coef_HEP_IBI{1, 3}(:, 3, 34))
[h,p,ci,stats] = ttest(coef_HEP_IBI{2, 2}(:, 2, 34), coef_HEP_IBI{2, 3}(:, 3, 34))


%% stats using permutation method
load coef_HEP_IBI_filt

% fix coefs variable
coef_HEP_IBI_fix = cell(2, 3);
for task = 1:3
    for grp = 1:2
        coef_HEP_IBI_fix{grp, task} = squeeze(coef_HEP_IBI{grp, task}(:, task, :));
    end
...
for task = 1:3
    for grp = 1:2
        [pval_t1(grp, task, :), t_orig_t1(grp, task, :), crit_t_t1(grp, task, :), est_alpha, seed_state]=mult_comp_perm_t1(coef_HEP_IBI_fix{grp, task});
    end
    [pval_t2(task, :), t_orig_t2(task, :), crit_t_t2(task, :), est_alpha, seed_state] = mult_comp_perm_t2(coef_HEP_IBI_fix{1, task},coef_HEP_IBI_fix{2, task});
    [pval_t1_all(task, :), t_orig_t1_all(task, :), crit_t_t1_all(task, :), est_alpha, seed_state]=mult_comp_perm_t1([coef_HEP_IBI_fix{1, task}; coef_HEP_IBI_fix{2, task}]);
end

[pval_t1_task, t_orig_t1_task, crit_t_t1_task, est_alpha, seed_state]=mult_comp_perm_t1([coef_HEP_IBI_fix{1, 3}; coef_HEP_IBI_fix{2, 3}]-[coef_HEP_IBI_fix{1, 2}; coef_HEP_IBI_fix{2, 2}]);


%% plot t-values from one sample t-test both groups together
load('E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\chanlocs.mat')
title_txt = {'Passive', 'Simple RT' 'Go/no-go'};%cmap = crameri('batlow');
for task = 1:3
    ...
    for x = 1:length(sig_chan_number)
        sig_chans_all{task}{x} = chanlocs(sig_chan_number(x)).labels;
    end
    
    figure; 
    if ~isempty(sig_chan_number)
        topoplot(ones(59, 1)*.025, chanlocs, 'electrodes', 'off', 'plotchans', sig_chan_number, 'style', 'blank',...
                'plotdisk', 'on',  'hcolor'  , 'none') ; hold on
    end
    topoplot(t_orig_t1_all(task, :), chanlocs, 'electrodes', 'off'); 
    % clim([0 5.5]); %c.Axis.FontSize = 16;
    colorbar;
...
%     set(get(gca,'title'),'Position',[0,-.65, 0])
    ...
    topoplot(mean([coef_HEP_IBI_fix{1, task}; coef_HEP_IBI_fix{2, task}], 1), chanlocs, 'electrodes', 'off'); 
    caxis([-.03 0]); %c.Axis.FontSize = 16;
    colorbar;
...
%% plot t-values from t-test comparison across groups
load('E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\chanlocs.mat')
title_txt = {'Passive', 'Simple RT' 'Go/no-go'};%cmap = crameri('batlow');
for task = 1:3
    ...
    topoplot(t_orig_t2(task, :), chanlocs, 'electrodes', 'off'); 
    % clim([0 5.5]); %c.Axis.FontSize = 16;
    colorbar;
    % colorbar('Ticks',[0, 2, 4], 'FontSize', 30, 'FontWeight','normal');
    colormap(crameri('imola')); % needs colour maps from http://www.fabiocrameri.ch/colourmaps.php
...
%%
channels = load('E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\chanlocs_with_ekg.mat');
tasks = {'passive', 'simple RT', 'go/no-go'};
load HEPTlock_IBI_tertls_nobsln_largepch_filt_cardiacICs;

x_axis=-199:2:1000; % in miliseconds

% load time windows where HEP correlates with IBI
mask_dir = 'E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_nobaseline_05HzFilter_cardiacICs\level2_onesample_IBIvsHEP_50-350ms';
mask_simpleRT = load([mask_dir, '\simpleRT\mask']);
mask_gng = load([mask_dir, '\gng\mask']);

for t = 1:length(tasks)
clear young_ibi_mean older_ibi_mean young_ibi_se older_ibi_se
    for ibi = 1:3
        young_ibi_mean(ibi, :, :) = squeeze(mean(HEP_T_locked_IBI_tertiles{1, 2}(:, t, ibi, :, 1:600), 1));
        older_ibi_mean(ibi, :, :) = squeeze(mean(HEP_T_locked_IBI_tertiles{2, 2}(:, t, ibi, :, 1:600), 1));
    
        young_ibi_se(ibi, :, :) = squeeze(std(HEP_T_locked_IBI_tertiles{1, 2}(:, t, ibi, :, 1:600), [], 1))/sqrt(size(HEP_T_locked_IBI_tertiles{1, 2}, 1));
        older_ibi_se(ibi, :, :)= squeeze(std(HEP_T_locked_IBI_tertiles{2, 2}(:, t, ibi, :, 1:600), [], 1))/sqrt(size(HEP_T_locked_IBI_tertiles{1, 2}, 1));
    end

    % channels_num = [7, 16, 25, 34];
    channels_num = 16%34;%[7, 25, 43, 57, 60];
    colormap cool;
    cmap = colormap;
    ...
    for p = channels_num
        ...
        for idx = -185:5:50
            plot([idx idx], [-1.45 1], 'color', [.9 .9 .9], 'LineWidth', 5)
            hold on
        end
...
%             hold on
        ...
        elseif t == 2 % simple RT
            mask = find(mask_simpleRT.mask(p, :) == 1);
             plot(x_axis(mask+125), ones(length(mask), 1) * -1, 's', 'MarkerSize', 5.5, 'color', 'k', ...
            'MarkerFaceColor', 'k')
        elseif t == 3 % gng
        ...
        hold off
        box off
        ax = gca; axis([-200 350 -1.50 1]);
%         if strcmp(name_channels, 'EKG')
%             axis([-inf inf -100 250])%axis([-inf .7 -200 200])
%         else
%             axis([-200 400 -1.50 1]);%axis([-inf .7 -1.50 1.50])
%         end
%         if t == 1
%             axis([-inf .7 -1.5 1])
%         else
...
tasks = {'passive', 'simple RT', 'go/no-go'};
load HEP_T_locked_IBI_tertiles_nobasln_largepoch_filt;
...
for t = 1:length(tasks)
clear ibi_mean ibi_se
    for ibi = 1:3
        ibi_mean(ibi, :, :) = squeeze(mean(cat(1, HEP_T_locked_IBI_tertiles{1, 2}(:, t, ibi, :, 1:600),...
            HEP_T_locked_IBI_tertiles{2, 2}(:, t, ibi, :, 1:600)), 1));
    
        ibi_se(ibi, :, :) = squeeze(std(cat(1, HEP_T_locked_IBI_tertiles{1, 2}(:, t, ibi, :, 1:600),...
            HEP_T_locked_IBI_tertiles{2, 2}(:, t, ibi, :, 1:600)), [], 1))/sqrt(size(HEP_T_locked_IBI_tertiles{1, 2}, 1));
    end
...
%             hold on
        ...
        ax = gca;
        if strcmp(name_channels, 'EKG')
            axis([50 400 -100 250])%axis([-inf .7 -200 200])
        else
            axis([-inf inf -2 1]);%axis([-inf .7 -1.50 1.50])
        end
        ax.LineWidth = 2.5;
        ax.FontSize = 28;
