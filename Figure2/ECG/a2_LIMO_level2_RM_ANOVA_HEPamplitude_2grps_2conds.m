...
level2_RM_ANOVA_dir = [pwd, '\level2_RM_ANOVA_ECG_grp_task_effect'];
mkdir(level2_RM_ANOVA_dir);
% effect of task and group on ECG amplitude - beta = 2:3
...
for f = 1:length(folder_list)
    if contains(folder_list(f).name, 'AB')
        ...
        Y(:, :, s, :) = Betas(:, 1:250, 1:2); % channels, time, subjects, measures
...
% level2_RM_ANOVA_dir = [pwd, '\level2_RM_ANOVA_IBIvsECG'];
% cd(level2_RM_ANOVA_dir);
