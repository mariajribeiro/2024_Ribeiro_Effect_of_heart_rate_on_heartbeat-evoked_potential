...
clear; close all
save_dir = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\eeg_derivatives';
%load eeg data and bad channels data
dirData = dir('D:\ProjectAgeingAuditoryTask\BIDS');      %# Get the data for the current directory
...
dirData_events = dir('D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\neweeg_events');      %# Get the data for the current directory
...
%load ica files
dirData_ica = dir('D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\files_with_ica');      %# Get the data for the current directory
dirIndex4 = [dirData_ica.isdir];
fileList_ica = {dirData_ica(~dirIndex4).name}';
%load artifacts
dirData_art_ics = dir('D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\artifactICs');      %# Get the data for the current directory
...
dir_artefact_seg = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\artefact_segments';

[ALLEEG EEG CURRENTSET ALLCOM] = eeglab; pop_editoptions( 'option_computeica', 1);
for s = 54%37:length(sbj_included)
...
%     fileList2 = {nextdirData2(~nextdirIndex2).name}';
   ...
   % calculated in D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\segments_with_artefacts.m
   ...
   for k = 1:length(fileList_eeg)
       if contains(fileList_eeg(k), 'eeg.set')
           ...
           % filter eeg data between .1 and 45 Hz
            EEG = pop_eegfiltnew(EEG, 0.1, 45, 16500, 0, [], 0);
...
%            [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3,'gui','off'); 
           EEG = eeg_checkset( EEG );
           basefilename = sprintf('eeg_Rpeak_Tpeak_%s.set', fileList_eeg{k});
