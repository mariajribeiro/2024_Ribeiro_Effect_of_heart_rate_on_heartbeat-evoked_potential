% LIMO level 1 effect of task and group on T-locked HEP - with random events
clear; close all;
...
%                 EEG = pop_select( EEG, 'nochannel',{'EKG'}); % INCLUDE EKG IN ANALYSES!!!!
        ...
        EEG = pop_epoch( EEG, {  'Tpeak'  }, [-0.2         .4], 'newname', 'Tpeak epochs all', 'epochinfo', 'yes');

        % HEP T-locked data - from 51 to 350 ms after the T peaks
        HEP_T_locked_random_data = cat(3, HEP_T_locked_random_data, EEG.data(1:59, 126:275, :));
