...
level2_RM_ANOVA_dir = [pwd, '\level2_RM_ANOVA_IBIvsHEP_3tasks'];
...
for f = 1:length(folder_list)
    if contains(folder_list(f).name, 'AB')
        ...
        Y(:, :, s, :) = Betas(:, :, 4:6); %channels, time, subjects, measures
...
% limo_random_robust(6,y,gp,factor_levels,LIMO,'go',option)
LIMOpath = limo_random_robust(6, Y, gp, 3, LIMO);
...
if ~isempty(mask)
    save mask_interaction mask
    save stat_values_interaction stat_values %F/t values
    save p_values_interaction p_values
