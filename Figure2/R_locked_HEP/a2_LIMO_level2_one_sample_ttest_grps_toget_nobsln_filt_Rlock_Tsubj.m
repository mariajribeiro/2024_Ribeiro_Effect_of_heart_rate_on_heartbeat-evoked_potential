...
group = {'young', 'older'};
level2_onesample_dir = [pwd, '\level2_onesample_HEP_grpstogeth_330-630ms_Tsubjs'];
...
for f = 1:length(folder_list)
    if contains(folder_list(f).name, 'AB')
        if ismember(str2double(folder_list(f).name(3:end)), groups{1}) && ~ismember(str2double(folder_list(f).name(3:end)), [21, 50, 55])
            load([folder_list(f).folder, filesep, folder_list(f).name, filesep, 'HEP_data.mat'])
            s = s + 1;
            Y(:, :, s) = squeeze(mean(HEP_data(:,15:164, :), 3)); % channels x time x subjects - from 51 to 400 ms locked with T peak
            subjects{s} = [folder_list(f).name];
        elseif ismember(str2double(folder_list(f).name(3:end)), groups{2}) && ~ismember(str2double(folder_list(f).name(3:end)), [21, 50, 55])
            load([folder_list(f).folder, filesep, folder_list(f).name, filesep, 'HEP_data.mat'])
            s = s + 1;
            Y(:, :, s) = squeeze(mean(HEP_data(:, 15:164, :), 3)); % channels x time x subjects - from 51 to 400 ms locked with T peak
            subjects{s} = [folder_list(f).name];
...
clear
load mask
[name, cluster] = limo_get_effect_size('one_sample_ttest_parameter_1.mat', mask)
% load(name)
