...
groups = {'young', 'older'};
dir_HR = 'E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_nobaseline_05HzFilter_cardiacICs\level2_regression_HR_avgtasks_50-350ms';
...
%         end
        ...
        if exist([mask_dir, '\mask_covariate.mat'])
            ...
            plot(x_axis(find(mask(chn, :) > 0)), ones(length(find(mask(chn, :) > 0)), 1)' * -.3, 'k', 'LineWidth', 4.5)
            hold on
        end
...
%% gng task
groups = {'young', 'older'};
dir_HR = 'E:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_nobaseline_05HzFilter_cardiacICs\level2_regression_HR';
for grp = 1:2
