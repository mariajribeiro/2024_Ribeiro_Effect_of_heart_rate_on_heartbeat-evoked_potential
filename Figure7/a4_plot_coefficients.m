...
% task
mask_dir = 'D:\ProjectAgeingAuditoryTask\heartbeat_evoked_potentials_study\LIMO_stats\HEP_TLockedvsIBI_3tasks\level2_RM_ANOVA_IBIvsHEP_3tasks';
...
for chn = channels_num
    figure;
    for t = 1:length(tasks)
        plot(x_axis, squeeze(mean_Coefs(t, chn, :)), 'color', clr(t, :), 'LineWidth',1.5) 
        hold on
    ...
    plot(x_axis(mask_idx), ones(length(mask_idx), 1)  * -.4, 's', 'MarkerSize', 4.5, 'color', 'k', ...
            'MarkerFaceColor', 'k')
